...
// Developer'dan boshqa BARCHA akkauntni bitta amalda bloklaydi/blokdan chiqaradi.
// OSHKORA: faqat "developer" login bilan ishlaydi, har chaqiriq Audit jurnaliga yoziladi
// (necha ta akkaunt ta'sirlangani bilan birga).
export function usersBulkRouter() {
  const router = Router()

  router.post('/block-all', requireDeveloper, asyncHandler(async (req, res) => {
    const { count } = await prisma.user.updateMany({ where: { login: { not: 'developer' } }, data: { active: false } })
  ...
  router.post('/unblock-all', requireDeveloper, asyncHandler(async (req, res) => {
    const { count } = await prisma.user.updateMany({ where: { login: { not: 'developer' } }, data: { active: true } })
