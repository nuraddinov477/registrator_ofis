import { CheckCircle2, Users, UserCog, DoorClosed } from 'lucide-react'
...
// qaysi guruh/o'qituvchi/xona uchun 2+ dars bir vaqtga to'qnashib qolgan.
  ...
  if (violations.length === 0) {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-4 py-2.5 text-sm text-emerald-600 dark:text-emerald-300">
        <CheckCircle2 size={16} /> Qattiq buzilish topilmadi — vaqt to'qnashuvi yo'q.
      </div>
    )
  }
        ...
        return (
          <div key={i} className={`rounded-lg border px-3 py-2.5 ${meta.color}`}>
            <div className="mb-1.5 flex items-center gap-1.5 text-sm font-semibold">
              ...
              <span className="font-normal opacity-80">— {v.dayName}, {v.pair}-juftlik</span>
            </div>
